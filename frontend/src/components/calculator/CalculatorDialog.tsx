...
export const CalculatorDialog: React.FC<CalculatorDialogProps> = ({ calculator }) => {
  ...
  return (
    <div
      ...
      className="fixed z-50 inset-x-0 bottom-0 sm:bottom-20 sm:right-6 sm:left-auto sm:w-[350px] transition-all duration-200 ease-out animate-in fade-in zoom-in-95 slide-in-from-bottom-4"
    >
      <div className="bg-card/95 backdrop-blur-xl border border-border/80 rounded-t-3xl sm:rounded-2xl shadow-2xl shadow-black/25 p-3.5 sm:p-4 space-y-2.5 max-h-[85vh] overflow-y-auto">
        {/* Header Bar */}
        <div className="flex items-center justify-between border-b border-border/40 pb-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-xs font-extrabold text-foreground flex items-center gap-1">
                Quick Calculator
                <Sparkles className="w-3 h-3 text-amber-500 fill-amber-500/20" />
              </h2>
              <p className="text-[9px] text-muted-foreground">TrackWise Expense Math</p>
            </div>
          </div>
          <button
            type="button"
