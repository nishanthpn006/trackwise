    ...
    >
      {context.toasts.map((toast) => {
        ...
        return (
          <div
            key={toast.id}
            className={`pointer-events-auto flex items-center justify-between gap-3 p-3.5 rounded-xl border shadow-lg transition-all duration-200 animate-in fade-in slide-in-from-bottom-3 ${
              isSuccess
          ...
          >
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              {!isSuccess && !isError && <Info className="h-5 w-5 text-primary shrink-0" />}
